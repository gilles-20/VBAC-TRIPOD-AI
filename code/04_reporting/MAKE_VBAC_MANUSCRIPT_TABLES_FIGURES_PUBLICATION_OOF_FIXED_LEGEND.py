...
Figure 4 Temporal discrimination and case-mix risk dispersion
...
OOF_FILE = RACE_RESULT_DIR / "TABLE_INTERNAL_VALIDATION_5FOLD.csv"
COMPARATIVE_OVERALL_FILE = (
    RACE_RESULT_DIR / "TABLE_RACE_NEUTRAL_VS_RACE_INCLUSIVE_OVERALL.csv"
)
LP_SPREAD_FILE = (
    RACE_RESULT_DIR / "TABLE_LINEAR_PREDICTOR_SPREAD_BY_PERIOD.csv"
)
...
print("\nCreating Table 2 from canonical OOF/comparative results...")

assert_file(OOF_FILE)
assert_file(COMPARATIVE_OVERALL_FILE)

oof = pd.read_csv(OOF_FILE)
perf = pd.read_csv(COMPARATIVE_OVERALL_FILE)
...
if set(oof["model"]) != expected_models or len(oof) != 2:
    raise ValueError(
        "TABLE_INTERNAL_VALIDATION_5FOLD.csv must contain exactly two rows: "
        "race-neutral and race-inclusive."
    )

dev_perf = perf.loc[
    perf["dataset"] == "development_5fold_internal_validation"
].copy()

if len(dev_perf) != 2:
    raise ValueError(
        "Comparative overall file must contain exactly two OOF development rows."
    )

check_cols = [
    "n", "vbac_events", "failed_tolac", "observed_rate", "mean_predicted",
    "auc", "brier", "calibration_intercept", "calibration_slope",
    "observed_expected",
]

for model in sorted(expected_models):
    a = oof.loc[oof["model"] == model].iloc[0]
    b = dev_perf.loc[dev_perf["model"] == model].iloc[0]
    for col in check_cols:
        if not np.isclose(float(a[col]), float(b[col]),
                          rtol=1e-10, atol=1e-12, equal_nan=True):
            raise ValueError(
                f"OOF source mismatch for {model}, {col}: "
                f"{a[col]} vs {b[col]}"
            )

main = perf.loc[
    perf["model"].isin(expected_models)
    & perf["dataset"].isin([
        "development_5fold_internal_validation",
...
if len(main) != 6:
    raise ValueError(f"Expected 6 rows for Table 2, found {len(main)}.")

period_map = {
    "development_5fold_internal_validation":
        "Development, 5-fold internal validation",
    "intermediate_validation_2022_2023":
        "Intermediate 2022–2023",
    "2024_posthoc_comparative_evaluation":
        "2024 comparative evaluation",
}
model_map = {
    "race_neutral_counselling_logistic": "Race-neutral",
    "race_inclusive_counselling_logistic": "Race-inclusive",
}

main["Model"] = main["model"].map(model_map)
main["Evaluation period"] = main["dataset"].map(period_map)

main.loc[
    (main["model"] == "race_neutral_counselling_logistic")
    & (main["dataset"] == "2024_posthoc_comparative_evaluation"),
    "Evaluation period",
] = "2024 final temporal test"

main.loc[
    (main["model"] == "race_inclusive_counselling_logistic")
    & (main["dataset"] == "2024_posthoc_comparative_evaluation"),
    "Evaluation period",
] = "2024 post-hoc comparison"

period_order = {
    "development_5fold_internal_validation": 0,
    "intermediate_validation_2022_2023": 1,
    "2024_posthoc_comparative_evaluation": 2,
}
model_order = {
    "race_neutral_counselling_logistic": 0,
    "race_inclusive_counselling_logistic": 1,
}
main["_period_order"] = main["dataset"].map(period_order)
main["_model_order"] = main["model"].map(model_order)
main = main.sort_values(["_period_order", "_model_order"]).reset_index(drop=True)

main["AUC (95% CI)"] = main.apply(
    lambda r: f"{r.auc:.3f} ({r.auc_lower:.3f}–{r.auc_upper:.3f})", axis=1
)
main["Brier (95% CI)"] = main.apply(
...
table2 = main[
    [
        "Model", "Evaluation period", "n",
        "AUC (95% CI)", "Brier (95% CI)",
...
table2_raw_path = TAB_DIR / "TABLE2_OVERALL_PREDICTIVE_PERFORMANCE_RAW.csv"
main.drop(columns=["_period_order", "_model_order"]).to_csv(
    table2_raw_path, index=False
)
print(f"  WROTE: {table2_raw_path}")

dev_neutral = oof.loc[
    oof["model"] == "race_neutral_counselling_logistic"
].iloc[0]
dev_inclusive = oof.loc[
    oof["model"] == "race_inclusive_counselling_logistic"
].iloc[0]

development_text = (
    "Five-fold internal validation of the development cohort produced "
    f"AUC {dev_neutral.auc:.4f} "
    f"(95% CI {dev_neutral.auc_lower:.4f}–{dev_neutral.auc_upper:.4f}) "
    "for the race-neutral model and "
    f"{dev_inclusive.auc:.4f} "
    f"(95% CI {dev_inclusive.auc_lower:.4f}–{dev_inclusive.auc_upper:.4f}) "
    "for the race/ethnicity-inclusive model."
)

(TAB_DIR / "MANUSCRIPT_DEVELOPMENT_OOF_SENTENCE.txt").write_text(
    development_text + "\n", encoding="utf-8"
)
print("  OOF development sentence:")
...
# ======================================================================
# 10. FIGURE 4 — TEMPORAL DISCRIMINATION AND RISK DISPERSION
# ======================================================================

print("\nCreating Figure 4: temporal discrimination and risk dispersion...")

assert_file(LP_SPREAD_FILE)
lp_spread = pd.read_csv(LP_SPREAD_FILE)

required_lp = {"period", "lp_sd", "lp_iqr", "lp_mean"}
missing_lp = required_lp - set(lp_spread.columns)
if missing_lp:
    raise ValueError(
        "Linear-predictor spread file is missing columns: "
        + ", ".join(sorted(missing_lp))
    )

neutral_perf = perf.loc[
...
auc_map = dict(zip(neutral_perf["dataset"], neutral_perf["auc"]))

spread_to_perf = {
    "development_2016_2021": "development_5fold_internal_validation",
    "intermediate_validation_2022_2023": "intermediate_validation_2022_2023",
    "2024_posthoc_comparative_evaluation":
        "2024_posthoc_comparative_evaluation",
}

lp_spread["performance_dataset"] = lp_spread["period"].map(spread_to_perf)
lp_spread["auc"] = lp_spread["performance_dataset"].map(auc_map)

figure4_data = lp_spread.set_index("period").loc[
    [
...
fig4_csv = TAB_DIR / "FIGURE4_TEMPORAL_AUC_AND_LP_SPREAD_DATA.csv"
figure4_data.to_csv(fig4_csv, index=False)
print(f"  WROTE: {fig4_csv}")

x = np.arange(len(figure4_data))
fig, ax1 = plt.subplots(figsize=(8.8, 5.8))

l1 = ax1.plot(
    x, figure4_data["auc"], marker="o", linewidth=2.1,
    markersize=6.5, label="Race-neutral AUC", zorder=4
)
ax1.set_ylabel("AUC")
ax1.set_xticks(x)
ax1.set_xticklabels(figure4_data["Period"])
ax1.set_xlabel("Evaluation period")
ax1.set_ylim(0.68, 0.72)
clean_axis(ax1, grid_axis="y")

ax2 = ax1.twinx()
l2 = ax2.plot(
    x, figure4_data["lp_sd"], marker="s", linewidth=1.9,
    linestyle="--", markersize=6.0, label="Linear-predictor SD", zorder=4
)
ax2.set_ylabel("SD of frozen race-neutral linear predictor")
ax2.spines["top"].set_visible(False)

for xi, auc in zip(x, figure4_data["auc"]):
    ax1.annotate(
        f"{auc:.4f}", (xi, auc), textcoords="offset points",
        xytext=(0, 8), ha="center", fontsize=9
    )

for xi, sd in zip(x, figure4_data["lp_sd"]):
    ax2.annotate(
        f"{sd:.3f}", (xi, sd), textcoords="offset points",
...
# Place the legend below the plotting area so it cannot overlap
# the 2024 AUC or linear-predictor SD annotations.
ax1.legend(
    lines,
    labels,
    loc="upper center",
...
ax1.set_title("Temporal discrimination and case-mix risk dispersion")

fig.text(
    0.5,
    0.015,
    (
        "Development AUC is 5-fold out-of-fold; "
        "linear-predictor spread is descriptive under the "
...
# Extra bottom margin accommodates the legend and methodological note.
fig.tight_layout(rect=[0, 0.14, 1, 0.98])
save_publication_figure(
    fig, "FIGURE4_TEMPORAL_DISCRIMINATION_RISK_DISPERSION"
)
plt.close(fig)


# ======================================================================
# 11. MANUSCRIPT-READY 2024 SUBGROUP TABLE
# ======================================================================
...
for filename in [
    "TABLE_INTERNAL_VALIDATION_5FOLD.csv",
    "TABLE_RACE_NEUTRAL_VS_RACE_INCLUSIVE_OVERALL.csv",
    "TABLE_LINEAR_PREDICTOR_SPREAD_BY_PERIOD.csv",
]:
    src = RACE_RESULT_DIR / filename
    if not src.exists():
        raise FileNotFoundError(f"Required comparative audit output missing: {src}")
    shutil.copy2(src, SUPP_DIR / filename)
    print(f"  COPIED: {filename}")
...
(MANUSCRIPT_DIR / "MANUSCRIPT_OUTPUT_MANIFEST.txt").write_text(
    textwrap.dedent(
        ...
        - TABLE2_OVERALL_PREDICTIVE_PERFORMANCE_RAW.csv
        - MANUSCRIPT_DEVELOPMENT_OOF_SENTENCE.txt
        - FIGURE4_TEMPORAL_AUC_AND_LP_SPREAD_DATA.csv
        ...
        - FIGURE4_TEMPORAL_DISCRIMINATION_RISK_DISPERSION.pdf/.tiff/_preview.png
